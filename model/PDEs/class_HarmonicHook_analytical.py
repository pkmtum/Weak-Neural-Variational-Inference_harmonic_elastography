...
class HarmonicHook():
    def __init__(self, options):
        ...
        self.rho = self.args_rhs["rho"]
        self.omega = self.args_rhs["frequency"] * 2 * 3.1415
    ...
    def _lhs(self, x, y, theta):
        ...
        integrant_i = torch.einsum('...ikj,wijk->...wi', sigma, dw) # [... x Nele x 2 x 2] x [... x Nele x 2 x 2] -> [... x Nele]
    ...
    def forward(self, x, y, theta):
        ...
        lhs2 = self._lhs2(y, theta)
        rhs = self._rhs(x, y, theta, self.args_rhs)
        if lhs.dim() == 2:
            rhs = rhs.unsqueeze(1)
        return lhs - lhs2 - rhs.squeeze()

    def _lhs2(self, y, theta):
        constant = self.rho * self.omega**2
        # calculate the values at the quadrature points
        u_Q = self.YToField.eval(y)
        w_Q = self.ThetaToField.eval(theta)
        # dot product of u_Q and w_Q
        u_dot_w_Q = torch.einsum('unij,wnij->uwni', u_Q, w_Q) # [... x Nele x Q]
        # sum over Q
        u_dot_w = torch.sum(u_dot_w_Q, dim=-1) # [... x Nele]
        # multiply with constant and weight (here 1/6 for all Q) and elementwise the Jacobian (here 2*A)
        integral_i = torch.tensor(1/6) * constant * torch.einsum('i,...i->...i', self.ThetaToField.Two_A, u_dot_w)
