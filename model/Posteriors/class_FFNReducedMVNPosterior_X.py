...
class FFNReducedMVNPosteriorX(approximatePosterior):
    def __init__(self, options):
        ...
        self.rescale_output = options.get("rescale_output", False)
        self.flag_exp_mu = options.get("flag_exp_mu", False)
    ...
    def sample(self, y, num_samples=None):
        ...
        if self.rescale_input_flag:
            y = (y - self.rescale_input_mean) / (self.rescale_input_std * 10)
        # depends on if we learn the covariance with a NN or not
        if self.learn_cov_seperatly:
            mu =  self.NN.forward(y)
            if self.flag_exp_mu:
                mu = torch.exp(mu)
            x = mu + torch.einsum('i,ji->ji',torch.exp(self.sigma_x), torch.randn(torch.Size((num_samples, self.dim_x)))) + torch.einsum('ik,lk->li', self.V_x, torch.randn(torch.Size((num_samples, self.reduced_dim_x)))) 
        else:
        ...
        if self.rescale_output:
            ...
            # x[x < 0] = 0
            # pass
        # make it more stable, perhaps?
        mask = x>10
        if torch.any(mask):
            warnings.warn("Some samples are too large. Clipping them to 10.")
            x[x>10] = torch.tensor(10.0) + torch.randn_like(x[mask])
        return x
