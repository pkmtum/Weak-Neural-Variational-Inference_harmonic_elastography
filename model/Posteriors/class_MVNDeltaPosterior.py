...
class MVNDeltaPosterior(approximatePosterior):
    ...
    def pyro_sampler(self, num_samples=None):
        if num_samples==None:
    ...
    def guide(self, theta=None):
        x_mean = pyro.param("x_mean", self.x_0)
        cov_x = pyro.param("cov_x", self.cov_x, constraint=dist.constraints.positive_definite)
        y_mean = pyro.param("y_mean", self.y_0)

        x = pyro.sample("x", dist.MultivariateNormal(x_mean, cov_x))
        y = pyro.sample("y", dist.Delta(y_mean, event_dim=1))

    def update_mapping(self, name, value):
        if name == "x_mean":
            self.x_0 = value.clone().detach()
        elif name == "y_mean":
            self.y_0 = value.clone().detach()
        elif name == "cov_x":
            self.cov_x = value.clone().detach()
