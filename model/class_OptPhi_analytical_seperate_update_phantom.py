...
class OptPhi_analytical_seperate:
    def __init__(self,
                 prior,
                 likeihood,
                 posterior,
        ...
        if self.likelihood_type == "Alternative":
            self.r_0 = r_0
        print("Using likelihood type: ", self.likelihood_type)
    ...
    def ELBO(self):
        ...
        elif self.samples_theta_kind == "Circular":
            res = self.PDE.forward(
                samples["x"], samples["y"], self.likelihood.get_circular_theta(num_samples=self.num_samples_theta))
        else:
            raise ValueError(
        ...
        if self.likelihood_type == "Gaussian":
            ...
            L_res = - Lambda/2 * torch.abs(res)
            L_res = torch.mean(torch.mean(L_res, dim=-1), dim=-1)
        elif self.likelihood_type == "Alternative":
            abs_res = torch.abs(res)
            # I suppose I do the likelihood on all residuals and then take the mean / sum
            mask = abs_res <= self.r_0
            L_res = - Lambda * (abs_res - self.r_0)
            L_res = L_res.masked_fill(mask, torch.tensor(0.0))
            L_res = torch.sum(torch.mean(L_res, dim=-1), dim=-1)  # expectation over samples, sum over thetas
        else:
            raise ValueError("Likelihood type not known.")

        # print("res: ", L_res/(-Lambda/2))
        # obervables Likelihood
        E_tau = self.posterior.Posteriors["tau"].expectation().to(u_filtered_flat.device)
        L_obs = torch.mean(dist.Normal(u_filtered_flat, E_tau**(-0.5)).log_prob(u_obs_flat).sum(
            dim=-1), dim=-1)  # sum over all observation points, average over all samples
        u_minus_uhat = u_filtered_flat - u_obs_flat
        ...
        return ELBO, elbo_parts, res.clone().detach(), samples, u_minus_uhat
    ...
    def run(self):
        ...
        # Do SVI
        elbo, elbo_parts, res_array, samples, u_minus_uhat = self.ELBO()

        # get the gradient
        elbo.backward(retain_graph=True)
        # clip gradients so nothing explodes
        torch.nn.utils.clip_grad_norm(self.posterior.Posteriors["x"].parameters(), 1e4)
        torch.nn.utils.clip_grad_norm(self.posterior.Posteriors["y"].parameters(), 1e4)
        ...
        return elbo.clone().detach(), current_grad, elbo_parts, Sqres, samples, u_minus_uhat
